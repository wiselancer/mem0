...
When searching hosted Mem0, prefer user scope first: `filters: {"user_id":"wiselancer"}`. If you need both personal and agent-specific memories, use `OR`, for example `filters: {"OR":[{"user_id":"wiselancer"},{"agent_id":"codex"}]}`. Do not use `AND` with `user_id` + `agent_id`; Mem0 stores entity scopes separately and that returns empty results.
