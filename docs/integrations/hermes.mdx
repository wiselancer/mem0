...
Hermes should use the same memory shape as other Mem0-enabled agents such as Claude Code, Codex, Cursor, and OpenClaw:

- Store durable facts, decisions, preferences, project state, environment setup, and reusable lessons.
- Skip raw transcripts, vague summaries, trivial interactions, and secrets.
- Use a stable `user_id` across trusted agents and `agent_id: hermes` to identify Hermes-written memories.
- Prefer self-contained facts with absolute dates when time matters.
- Use `mem0_conclude` for curated facts that should be stored exactly as written.

Example curated memory:
